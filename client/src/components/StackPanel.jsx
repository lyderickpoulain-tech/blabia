...
export default function StackPanel({ project, toolbox, overrides, onSaveOverrides, onClose }) {
  ...
  const buildSummary = () => {
    ...
    for (const cat of TOOLBOX_CATEGORIES) {
      ...
      if (!eff) continue;
      const list = eff.status === 'planned' ? planned : eff.status === 'evaluating' ? evaluating : owned;
      list.push(eff.label);
    }
  ...
  const handleSuggest = async () => {
    ...
    setSuggestion('');
    try {
      const { data } = await api.post(`/projects/${project.id}/suggest-stack`, {
        toolboxSummary: buildSummary()
      });
      setSuggestion(data.suggestion);
    } catch {
      setSuggestion('Erreur lors de la suggestion — réessayez.');
    } finally {
      setSuggesting(false);
    }
  };
  ...
  const handleExport = async () => {
    ...
    for (const cat of TOOLBOX_CATEGORIES) {
      const eff = getEffective(cat.id);
      if (!eff) continue;
      const status = STATUS_LABELS[eff.status] || 'Possédé';
      lines.push(`${cat.icon} ${cat.label} : ${eff.label} (${status})`);
    }
    try {
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {eff ? (
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {hasOv && (
                        <span className="text-[10px] text-violet-500 shrink-0 font-medium">ajusté</span>
                      )}
                    </div>
                  ) : (
